-- database_schema_sqlserver_fixed.sql
-- Script SQL Server corrigé pour le système de gestion des pannes IT AMNAL
...
-- Supprimer les vues
IF EXISTS (SELECT * FROM sys.views WHERE name = 'vue_stats_journalieres')
    DROP VIEW vue_stats_journalieres;
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = 'vue_tickets_complets')
    DROP VIEW vue_tickets_complets;
GO
...
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'pieces_jointes')
...
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'historique_tickets')
    DROP TABLE historique_tickets;
GO

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'statistiques_tickets')
    DROP TABLE statistiques_tickets;
GO
...
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'categories_pannes')
    DROP TABLE categories_pannes;
GO
...
PRINT 'Toutes les tables, vues, procédures et triggers ont été créés correctement';
GO
